...
namespace ProcessMonitorTest
{
    ...
    {
        /// <summary>
        ///     Use the Process.GetProcessesByName method in C# to get a list of all the processes that match the name of the
        ...
        ///     You can create counters for CPU usage (Processor\% Processor Time), working set memory usage (Process\Working Set),
        ///     and private memory usage (Process\Private Bytes).
        ///     Use the PerformanceCounter.NextValue method in a loop to continuously capture the values of the performance
        ...
        public static void Monitor(params string[] args)
        {
            ...
            var cpuCounters = new PerformanceCounter[processes.Length];
            var workingSetCounters = new PerformanceCounter[processes.Length];
            var privateBytesCounters = new PerformanceCounter[processes.Length];

            for (var i = 0; i < processes.Length; i++)
            {
                cpuCounters[i] =
                    new PerformanceCounter("Processor", "% Processor Time", processes[i].ProcessName, true);
                workingSetCounters[i] =
                    new PerformanceCounter("Process", "Working Set", processes[i].ProcessName, true);
                privateBytesCounters[i] =
                    new PerformanceCounter("Process", "Private Bytes", processes[i].ProcessName, true);
            }
            ...
            while (true)
            {
                for (var i = 0; i < processes.Length; i++)
                {
                    ...
                    var workingSet = workingSetCounters[i].NextSample().RawValue;
                    var privateBytes = privateBytesCounters[i].NextSample().RawValue;
