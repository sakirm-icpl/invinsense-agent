...
echo %DATETIME% active-response/bin/full-scan.exe: Scan starting... >> %LOGFILE%

rem Run the command and log the output with timestamps, excluding scan start logs
for /f "tokens=*" %%a in ('%DEFENDER% -Scan -ScanType 2 2^>^&1') do (
    echo %%a | findstr /C:"Scan starting..." > nul || (
        echo %DATETIME% active-response/bin/full-scan.exe: %%a >> %LOGFILE%
    )
)
