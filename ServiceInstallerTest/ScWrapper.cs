...
namespace ServiceInstallerTest
{
    internal static class ScWrapper
    {
        ...
        public static void Install(string path, string name)
        {
            ...
            {
                using (var svc = new ServiceInstaller())
                {
                    installer.Account = ServiceAccount.LocalSystem;
                    installer.Username = null;
                    installer.Password = null;

                    svc.Context = new InstallContext($"install_{name}.log",
                        new[] { string.Format("/assemblypath={0}", path), "logConsole=false" });
        ...
        public static void Uninstall(string path, string name)
        {
            ManagedInstallerClass.InstallHelper(new[]
                { "/u", "C:\\Code\\invinsense-agent\\SampleService\\bin\\Debug\\SampleService.exe" });

            using (var installer = new ServiceProcessInstaller())
            {
                using (var svc = new ServiceInstaller())
                {
                    svc.Context = new InstallContext($"uninstall_{name}.log",
                        new[] { string.Format("/assemblypath={0}", path), "logConsole=false" });
                    svc.ServiceName = name;
                    svc.Parent = installer;
                    svc.Uninstall(null);
